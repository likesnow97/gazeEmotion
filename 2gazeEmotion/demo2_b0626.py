    ...
    while True:
        ...
        for box in bboxs:
            ...
            if 1:
                ...
                class_names = ['Stress', 'Disgust', 'Anxiety', 'Happy', 'Depression', 'Anxiety or Stress', 'Neutral']
                # class_names = ['Angry', 'Disgust', 'Fear', 'Happy', 'Sad', 'Surprise', 'Neutral']
                predicted = emotions[face_counter]
                emotion_counter[predicted] = emotion_counter[predicted] + 1
                emotion_counter = torch.tensor(emotion_counter)
                total_, totalpre = torch.max(emotion_counter, 0)
                face_counter += 1
                ...
                cv2.putText(frame, class_names[int(predicted.cpu().numpy())], (x - 20, y - 20),
                            ...
                            (0, 255, 0), 1)
                cv2.putText(frame, 'total_max:' + class_names[int(totalpre.cpu().numpy())], (x - 40, y - 40),
            ...
            print(str(time.time()), file=data)
            print(name, file=data)
            print(class_names[int(predicted.cpu().numpy())], file=data)
            if face_frame_counter % 5 == 0:
                print('===Total===:'+class_names[int(totalpre.cpu().numpy())], file=data)
            # data.write()
            data.close()
        if len(bboxs) != 0:
